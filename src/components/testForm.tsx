...
export default function InfluencerForm({ campaignId }: { campaignId: string }) {
  const [campaignFormFields, setCampaignFormFields] =
    useState<InfluencerCampaignForm | null>(null);
  const [fetchError, setFetchError] = useState<string | null>(null);

  useEffect(() => {
    async function fetchCampaignForm() {
      try {
        const data = await getCampaignForm(campaignId);
        setCampaignFormFields(data);
      } catch (error) {
        setFetchError(
  ...
  async function formSubmitFn(data: any) {
    const formattedData: InfluencerCampaignForm = {
      ...
      custom_fields: campaignFormFields!.custom_fields.map((field, index) => ({
        ...field,
  ...
  if (fetchError)
    return <div className="min-h-screen text-red-500">{fetchError}</div>;

  if (!campaignFormFields)
